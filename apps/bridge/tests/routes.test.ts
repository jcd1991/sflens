...
const tooling = {
  query: async (soql: string) => { calls.push(soql); if (soql.includes("TraceFlag")) return { records: traceActive ? [{ Id: "7tfTEST", ExpirationDate: "2099-01-01T00:00:00Z" }] : [] }; if (soql.includes("DebugLevel")) return { records: [{ Id: "7dlTEST" }] }; if (soql.includes("User")) return { records: [{ Id: "005TEST" }] }; return { records: [{ Id: "07LTEST", StartTime: "2026-01-01T00:00:00.000Z", LogUser: { Name: "Dev" }, Operation: "/apex/Test", Status: "Success", LogLength: 123 }] }; },
...
describe("bridge routes", () => {
  ...
  it("retrieves a body through the Tooling API and limits async ids", async () => {
    const body = await get("/orgs/test-org/logs/07LTEST/body");
    expect(await body.json()).toContain("USER_DEBUG");
    await get(`/orgs/test-org/async-jobs?ids=${Array.from({ length: 30 }, (_, i) => `00JOB${i}`).join(",")}`);
