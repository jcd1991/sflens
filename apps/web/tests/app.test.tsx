...
describe("SF Lens browser workflow", () => {
  it("prompts for Salesforce authorization and places Demo below the auth choice", () => { render(<App />); expect(screen.getByText(/Sign in to inspect recent debug logs/)).toBeInTheDocument(); const authorize = screen.getByRole("button", { name: "Authorize Salesforce" }); const demo = screen.getByRole("button", { name: /Continue in Demo mode/ }); expect(authorize).toBeInTheDocument(); expect(demo).toBeInTheDocument(); expect(authorize.compareDocumentPosition(demo) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy(); expect(screen.queryByLabelText("Startup token")).not.toBeInTheDocument(); });
  ...
  it("makes Demo a visible workspace reset", async () => { render(<App />); fireEvent.click(screen.getByRole("button", { name: /Continue in Demo mode/ })); await waitFor(() => expect(screen.getByText("Demo log loaded")).toBeInTheDocument()); expect(screen.getByText("LOCAL-ONLY")).toBeInTheDocument(); });
